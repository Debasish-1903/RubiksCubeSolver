...
bool PatternDatabase ::setNumMoves(const uint32_t idx, const uint8_t numMoves) {
    uint8_t oldMoves =this->getNumMoves(idx);

    if(oldMoves==0xF){  //check if oldMoves is unset or set
        ++this->numItems;
    }
...
bool PatternDatabase ::setNumMoves(const RubiksCube33 &cube, const uint8_t numMoves) {
...
uint8_t PatternDatabase::getNumMoves(const RubiksCube33 &&cube) const{
         return this->database.get(this->getDatabaseIndex(cube));
}
...
void PatternDatabase::toFile(const string &filePath) const {
   ...
   writer.write(
           reinterpret_cast<const char*>(this->database.data()), this->database.storageSize()
           );
...
bool PatternDatabase::fromFile(const string &filePath) {
    ...
    reader.read(
            reinterpret_cast<char*>(this->database.data()),this->database.storageSize()
    );
